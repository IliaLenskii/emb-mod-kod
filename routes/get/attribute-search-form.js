...
module.exports = (req, res) => {
    ...
    ( async function() {
        ...
        req.userEvent( 'Search-event', { comment: i18n.__('Attribute-search') + ' '
            + Object.keys( params )
                .filter( k => params[ k ].value || params[ k ].values )
                .map( f => getFormElem( f ).name.$value + ': ' + ( params[ f ].value || params[ f ].values ) ).join( ', ' ) } );
                
        let search = await client.AttrSearchAsync( { arg0: client.getConditions(), arg1: null, arg2: req.query.transneftId }, {
        ...
        for( let i = 0; i < search[ 0 ].DocListInfo.parts.$value; i++ ) {
            let arrLs2 = search[ 0 ].DocListInfo.size.$value == 0
                ? []
                : ( await client.GetSearchListAsync( { 'arg0': search[ 0 ].DocListInfo.id.$value, 'arg1': null, 'arg2': i, arg3: null, arg4: 'abc' }, { useEmptyTag: true, postProcess: xml => { xml = xml.replace( '<arg1>', '<arg1 xsi:nil="true">' ); return xml; }, timeout: 120000 /*, proxy: 'http://localhost:' + global.KServerApi.VDirInfo.localPort*/ } ) )
                    [ 0 ].ArrayOfDocListItem.item_DocListItem.map( r => { return {
        ...
        if ( arrLs.length > total ) arrLs = arrLs.reduce( ( o, d ) => { if ( o.filter( nd => nd.nd == d.nd ).length == 0 ) o.push( d ); return o; }, [] );
        for ( let d in arrLs ) {
